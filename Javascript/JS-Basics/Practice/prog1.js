...
// --------------------------------------------------------------------
let data = ["Hare", "Krishna", "Hare", "Krishna",
    "Krishna", "Krishna", "Hare", "Hare", ":-O"
];
...
let user = people.slice().sort((a, b) => a.age - b.age).map(item => item.name)
...
function filterData(keyProp) {
    typeof data[0][keyProp] !== 'string' ? data.sort((a, b) => a[keyProp] - b[keyProp]) :
...
const verifyKeyExistence = () => (parentObj.reduce((prev, curr) => {
...
let uniqueArray = () => (user.reduce((prev, curr) => {
...
const removeValues = (...input) => {
    user.filter(item => input.includes(item))
        .map(item => user.splice(user.indexOf(item), 1))
    return user
}
...
let uniqueArray = () => (newArr.reduce((acc, item) => {
...
const repetitiveCount = () => (user1.reduce((prev, curr) => {
...
const removeDuplicateArray = user.filter((item, index) => user.indexOf(item) == index)
const repeatedValue = user.filter((item, index) => user.indexOf(item) !== index)
...
function solution(A) {
    ...
    return newArr.length > 1 ? new String(newArr).split(',').join('').length : 0
